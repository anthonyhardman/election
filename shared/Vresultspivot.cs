using System;
using System.Collections.Generic;

namespace shared
{
    public partial class Vresultspivot
    {
        public int? ElectionId { get; set; }
        public string? OfficeName { get; set; }
        public int? OfficeId { get; set; }
        public string? CandidateName { get; set; }
        public int? CandidateId { get; set; }
        public int? Initialroundvotes { get; set; }
        public string? Initialroundpct { get; set; }
        public int? Round1votes { get; set; }
        public string? Round1pct { get; set; }
        public int? Round2votes { get; set; }
        public string? Round2pct { get; set; }
        public int? Round3votes { get; set; }
        public string? Round3pct { get; set; }
        public int? Round4votes { get; set; }
        public string? Round4pct { get; set; }
        public int? Round5votes { get; set; }
        public string? Round5pct { get; set; }
        public int? Round6votes { get; set; }
        public string? Round6pct { get; set; }
        public int? Round7votes { get; set; }
        public string? Round7pct { get; set; }
        public int? Round8votes { get; set; }
        public string? Round8pct { get; set; }
        public int? Round9votes { get; set; }
        public string? Round9pct { get; set; }
    }
}
